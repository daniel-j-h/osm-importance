...
.PHONY: osrm
osrm:
	@docker run -it --rm -v "$(shell pwd):/data:z" ghcr.io/project-osrm/osrm-backend:v5.27.1 osrm-extract -p /data/profiles/bicycle.lua /data/berlin-latest.osm.pbf
...
.PHONY: api
api:
	@docker run -it --rm -v "$(shell pwd):/data:z" -p 127.0.0.1:5001:5001 ghcr.io/project-osrm/osrm-backend:v5.27.1 osrm-routed --ip 0.0.0.0 --port 5001 -a mld --max-alternatives 100 /data/berlin-latest.osrm
